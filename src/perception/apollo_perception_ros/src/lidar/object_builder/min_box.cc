...
#include "common/convex_hullxy.h"
...
void MinBoxObjectBuilder::ComputePolygon2dxy(std::shared_ptr<Object> obj) {
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr pcd_xy(new pcl::PointCloud<pcl::PointXYZI>);
  ...
  ConvexHull2DXY<pcl::PointXYZI> hull;
  ...
  if (poly_vt.size() == 1u) {
    //std::cout << "poly_vt.size() == 1u" << std::endl;
    std::vector<int> ind(poly_vt[0].vertices.begin(),
                         poly_vt[0].vertices.end());
    util::TransformPointCloud(plane_hull, ind, &obj->polygon);
  } else {
