...
#include "lidar/tracker/feature_descriptor.h"
#include "lidar/tracker/hungarian_matcher.h"
#include "lidar/tracker/kalman_filter.h"
#include "lidar/tracker/track_object_distance.h"

namespace apollo_perception_standalone {

bool HmObjectTracker::Init(std::string &file_path_hm_tracker_config) {
  // Initialize tracker's configs
  using util::GetProtoFromFile;

  if (!GetProtoFromFile(file_path_hm_tracker_config, &config_)) {
    AERROR << "Cannot get config proto from file: " << file_path_hm_tracker_config;
    return false;
  }
...
bool HmObjectTracker::Track(
    ...
    bool log_lidar) {
  //AINFO << "track timestamp: " << std::setprecision(6) << timestamp / 1e6;
  // A. track setup
  ...
  ConstructTrackedObjects(objects, &transformed_objects, velo2world_pose,
                          options);
  /*
  for (int idx = 0; idx < transformed_objects.size(); idx++) {
    AINFO << "transformed_objects id " << idx 
          << " old direction " << transformed_objects[idx]->object_ptr->direction(0) 
          << " " << transformed_objects[idx]->object_ptr->direction(1)  
          << " " << transformed_objects[idx]->object_ptr->direction(2)
          << " new direction " << transformed_objects[idx]->direction(0) 
          << " " << transformed_objects[idx]->direction(1)  
          << " " << transformed_objects[idx]->direction(2);
  }
  */
  // C. prediction
  ...
  AWARN_IF(log_lidar) << "********** Lidar matching:" 
        << " tracks.size() " << tracks.size() 
        << " transformed_objects.size() " << transformed_objects.size() 
        << " assignments.size() " << assignments.size() 
        << " unassigned_objects.size() " << unassigned_objects.size() 
        << " unassigned_tracks.size() " << unassigned_tracks.size()
        << " time_diff " << time_diff;
...
void HmObjectTracker::ConstructTrackedObjects(
  ...
  for (int i = 0; i < num_objects; ++i) {
    ...
    obj->clone(*objects[i]);
    //AINFO << "obj id " << i << " direction " << objects[i]->direction(0) << " " << objects[i]->direction(1)  << " " << objects[i]->direction(2);
    ...
    // Transforming all tracked objects using pose
...
void HmObjectTracker::TransformTrackedObject(
  ...
  // Eigen::Vector3f& anchor_point = (*obj)->anchor_point;
  // anchor_point =
  //     (pose * Eigen::Vector4d(anchor_point(0), anchor_point(1), anchor_point(2), 1))
  //         .head(3)
  //         .cast<float>();
  // // transform velocity
  // Eigen::Vector3f& velocity = (*obj)->velocity;
  // velocity =
  //     (pose * Eigen::Vector4d(velocity(0), velocity(1), velocity(2), 1))
  //         .head(3)
  //         .cast<float>();
}
...
void HmObjectTracker::TransformObject(std::shared_ptr<Object>* obj,
  ...
  util::TransformPointCloud<pcl_util::Point>(pose, (*obj)->cloud);
  util::TransformPointCloud<pcl_util::PointD>(pose, &((*obj)->polygon));
}
...
void HmObjectTracker::CreateNewTracks(
  ...
  for (size_t i = 0; i < unassigned_objects.size(); ++i) {
    ...
    //AINFO << "new obj id " << obj_id << " direction " << new_objects[obj_id]->direction(0) << " " << new_objects[obj_id]->direction(1)  << " " << new_objects[obj_id]->direction(2);
