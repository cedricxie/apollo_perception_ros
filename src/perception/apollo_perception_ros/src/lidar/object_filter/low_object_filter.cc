...
bool LowObjectFilter::Init(std::string &file_path_low_object_filter_config) {
  // std::cout << "Start Init of LowObjectFilter " << std::endl;
  if (!util::GetProtoFromFile(file_path_low_object_filter_config, &config_)) {
    AERROR << "Cannot get config proto from file: "
           << file_path_low_object_filter_config;
    return false;
  }
