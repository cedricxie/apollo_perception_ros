...
bool ProbabilisticFusion::Init() {
  AERROR << "ProbabilisticFusion should be initialized from passed argument!";
  return false;
  // //sensor_manager_ = PbfSensorManager::Instance();
  // sensor_manager_ = new PbfSensorManager();
  ...
  // AINFO << "Initialized ProbabilisticFusion successfully";
  // return true;
}

bool ProbabilisticFusion::Init(bool log_fusion) {
  AERROR << "ProbabilisticFusion should be initialized from passed argument!";
  return false;
  // log_fusion_ = log_fusion;
  // // AWARN_IF(log_fusion_) << "Fusion: prob fusion initialization started";
...
bool ProbabilisticFusion::Init(bool log_fusion, std::string &file_path_config) {
  ...
  std::string file_path_config_prob_fusion = 
    file_path_config + "/config/probabilistic_fusion_config.pb.txt";

  CHECK(GetProtoFromFile(file_path_config_prob_fusion, &config_));
