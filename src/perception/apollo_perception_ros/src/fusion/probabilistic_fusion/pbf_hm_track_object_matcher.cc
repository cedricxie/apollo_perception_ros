...
#include "fusion/probabilistic_fusion/pbf_track_object_distance.h"
...
bool PbfHmTrackObjectMatcher::Match(
  ...
  // AWARN  << "After IdAssign: "
  //        << "Num of fusion tracks = " << fusion_tracks.size()
  ...
  // global idx (idx in fusion_tracks) -> local idx (idx in unassigned_fusion_tracks)
  ...
  // global idx (idx in sensor_objects) -> local idx (idx in unassigned_sensor_objects)
  ...
  for (size_t i = 0; i < unassigned_fusion_tracks->size(); ++i) {
    const int track_ind = unassigned_fusion_tracks->at(i);  // global idx
    const int track_ind_loc = track_ind_g2l[track_ind];  // local idx
...
void PbfHmTrackObjectMatcher::ComputeAssociationMat(
  ...
  for (size_t i = 0; i < unassigned_fusion_tracks.size(); ++i) {
    ...
    for (size_t j = 0; j < unassigned_sensor_objects.size(); ++j) {
      ...
      // AWARN  << "ComputeAssociationMat: "
      //        << "sensor distance: " << distance 
      //        << " between tracker of ID: " << fusion_track->GetTrackId()
      //       << " and object sensor_idx: " << sensor_idx;
...
bool PbfHmTrackObjectMatcher::HmAssign(
  ...
  }  // end of loop on fusion_components
...
void PbfHmTrackObjectMatcher::MinimizeAssignment(
  ...
  util::HungarianOptimizer hungarian_optimizer(association_mat);
...
void PbfHmTrackObjectMatcher::ComputeConnectedComponents(
  ...
  util::ConnectedComponentAnalysis(nb_graph, &components);
