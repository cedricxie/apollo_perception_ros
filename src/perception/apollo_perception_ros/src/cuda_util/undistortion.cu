...
#include "cuda_util/undistortion.h"
...
int ImageGpuPreprocessHandler::init(const std::string &intrinsics_path, int dev) {
    ...
    CUDA_CHECK_APOLLO(cudaMalloc(&_d_rgb, _out_size)); // if input is rgb, its size is same as output's
    CUDA_CHECK_APOLLO(cudaMalloc(&_d_dst, _out_size));
    ...
    CUDA_CHECK_APOLLO(cudaMalloc(&_d_mapx, img_size * sizeof(float)));
    CUDA_CHECK_APOLLO(cudaMalloc(&_d_mapy, img_size * sizeof(float)));

    CUDA_CHECK_APOLLO(cudaMemcpy(_d_mapx, mapx.data(), img_size * sizeof(float),
                cudaMemcpyHostToDevice));
    CUDA_CHECK_APOLLO(cudaMemcpy(_d_mapy, mapy.data(), img_size * sizeof(float),
                cudaMemcpyHostToDevice));
    _inited = true;
    return 0;
}
...
int ImageGpuPreprocessHandler::handle(uint8_t *src, uint8_t *dst) {
    ...
    CUDA_CHECK_APOLLO(cudaMemcpy(_d_rgb, src, _in_size, cudaMemcpyHostToDevice));
    ...
    NppStatus eStatusNPP = nppiRemap_8u_C3R (_d_rgb, Remapsize, (_width * CHANNEL), RemapRect,
            _d_mapx, (_width * sizeof(float)), _d_mapy, (_width * sizeof(float)), _d_dst,
            (_width * CHANNEL), Remapsize, RemapMode );

    if (eStatusNPP != NPP_SUCCESS) {
        std::cerr << "NPP_CHECK_NPP - eStatusNPP = " << eStatusNPP << std::endl;
        return (int)eStatusNPP;
    }

    CUDA_CHECK_APOLLO(cudaMemcpy(dst, _d_dst, _out_size, cudaMemcpyDeviceToHost));
    return 0;
}
...
int ImageGpuPreprocessHandler::release(void) {
    if (_d_mapy) {
        CUDA_CHECK_APOLLO(cudaFree(_d_mapy));
        _d_mapy = NULL;
    }
    if (_d_mapx) {
        CUDA_CHECK_APOLLO(cudaFree(_d_mapx));
        _d_mapx = NULL;
    }
    if (_d_dst) {
        CUDA_CHECK_APOLLO(cudaFree(_d_dst));
        _d_dst = NULL;
    }
    if (_d_rgb) {
        CUDA_CHECK_APOLLO(cudaFree(_d_rgb));
        _d_rgb = NULL;
    }
    _inited = false;
    return 0;
...
int ImageGpuPreprocessHandler::load_camera_intrinsics(
	...
	std::cout << "File not exists: " << intrinsics_path;
	return 1;
    }

    YAML::Node node = YAML::LoadFile(intrinsics_path);
    if (node.IsNull()) {
		std::cout << "Load " << intrinsics_path << " failed! please check!";
        return -1;
    }
