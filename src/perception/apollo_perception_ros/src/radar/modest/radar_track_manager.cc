...
void RadarTrackManager::Update(SensorObjects *radar_obs) {
  ...
  AssignTrackObsIdMatch(*radar_obs, &assignment, &unassigned_track,
                        &unassigned_obs);
  // AWARN << "++++++++++ Radar matching" 
  //         << ": assignment " << assignment.size()
  //         << "; unassigned_track " << unassigned_track.size()
  //         << "; unassigned_obs " << unassigned_obs.size();
...
void RadarTrackManager::UpdateAssignedTrack(
  ...
  for (size_t i = 0; i < assignment.size(); i++) {
    int id = obs_tracks_[assignment[i].first].GetObsId();
    double timestamp_old = obs_tracks_[assignment[i].first].GetTimestamp();
    double time_diff = fabs(radar_obs.timestamp - timestamp_old);
    // AWARN_IF(log_radar_) << "Radar track: UpdateAssignedTrack id " << id
    //           << " " << obs_tracks_[assignment[i].first].GetObsRadar()->track_id
    //           << std::fixed << std::setprecision(12)
    //           << ", old at: " << timestamp_old
    //           << ", new at: " << radar_obs.timestamp
    //           << ", time diff: " << time_diff;
...
void RadarTrackManager::UpdateUnassignedTrack(
  ...
  for (size_t i = 0; i < unassigned_track.size(); i++) {
    if (obs_tracks_[unassigned_track[i]].GetObsRadar() != nullptr) {
      int id = obs_tracks_[unassigned_track[i]].GetObsId();
      double timestamp_old = obs_tracks_[unassigned_track[i]].GetTimestamp();
      double time_diff = fabs(timestamp - timestamp_old);
      // AWARN_IF(log_radar_) << "Radar track: UpdateUnassignedTrack id " << id
      //         << " " << obs_tracks_[unassigned_track[i]].GetObsRadar()->track_id
      //         << std::fixed << std::setprecision(12)
      //         << ", old at: " << obs_tracks_[unassigned_track[i]].GetTimestamp()
      //         << ", new at: " << timestamp
      //         << ", time diff: " << time_diff;
      if (time_diff > RADAR_TRACK_TIME_WIN) {
        AWARN_IF(log_radar_) << "\033[1;31mRadar track deleted id " << id << ", " 
        << obs_tracks_[unassigned_track[i]].GetObsRadar()->track_id << "\033[0m\n";
...
void RadarTrackManager::CreateNewTrack(const SensorObjects &radar_obs,
  ...
  for (size_t i = 0; i < unassigned_obs.size(); i++) {
    obs_tracks_.push_back(RadarTrack(*(radar_obs.objects[unassigned_obs[i]]),
                                     radar_obs.timestamp));
    int id = obs_tracks_.back().GetObsId();
    AWARN_IF(log_radar_) << "\033[1;32mRadar track created id " << id << ", "
    << radar_obs.objects[unassigned_obs[i]]->track_id << "\033[0m\n";
  }
}
