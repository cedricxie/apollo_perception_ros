...
bool ObjectBuilder::Build(const ContiRadar &raw_obstacles,
  ...
  for (int i = 0; i < raw_obstacles.contiobs_size(); ++i) {
    ...
    // calculate continuously tracked num and compare with delay_frames_
    ...
    if (use_fp_filter_ &&  // turned off in config file
    ...
    // check for velocity conflict; skipped when main_velocity is not set
    ...
    object_ptr->score_type = PerceptionObstacle::CONFIDENCE_RADAR;
    object_ptr->score =
  ...
  }  // loop of i between [0, raw_obstacles.contiobs_size())
