...
bool FlatCameraTransformer::Transform(
  ...
  // Center in Camera Frame and Derived Center in Ego Car Frame
  ...
  for (auto obj_ptr : *objects) {
    ...
    // Get 2D direction vector in top down view
...
bool FlatCameraTransformer::Transform(
    std::vector<std::shared_ptr<VisualObject>> *objects, 
    std::shared_ptr<Eigen::Matrix4d> &trans) {
  if (!objects) return false;

  // Pairs of 
  //   1. Center in Camera Frame and,
  //   2. Derived Center in Ego Car Frame
  std::vector<std::pair<Eigen::Vector3f, Eigen::Vector3f>> centers;

  // camera2car_: trans from pandora cam to hesai40
  // trans: trans from hesai40 to vehicle
  Eigen::Matrix4f lidar2veh = (*trans).cast<float>();
  Eigen::Matrix4f cam2veh = lidar2veh * camera2car_;
  Eigen::Matrix<float, 3, 1> cam2veh_flat_offset =
      Eigen::Matrix<float, 3, 1>(cam2veh(0, 3), cam2veh(1, 3), 0.0f);

  for (auto obj_ptr : *objects) {
    // Get flat 2D distance in meter
    float d = obj_ptr->distance;
    float d_v = std::abs(cam2veh(2, 3) - obj_ptr->height / 2.0f);
    float d_flat = sqrt(d * d - d_v * d_v);

    // Get 2D direction vector in top down view
    Eigen::Vector3f center = obj_ptr->center;  // Center in Camera Space
    Eigen::Vector4f center_v(center.x(), center.y(), center.z(), 0.0f);
    center_v = cam2veh * center_v;
    center_v.z() = 0.0f;
    Eigen::Vector3f unit_v_flat = MakeUnit(center_v.head(3));

    // 2D position in top-down view of ego-car space
    Eigen::Vector3f pos_ground = unit_v_flat * d_flat;
    obj_ptr->center = pos_ground + cam2veh_flat_offset;
