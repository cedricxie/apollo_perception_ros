...
bool load_types(const string &path, vector<ObjectType> *types) {
  const map<string, ObjectType> type_map = {
      {"UNKNOWN", apollo_perception_standalone::ObjectType::UNKNOWN},
      {"UNKNOWN_MOVABLE", apollo_perception_standalone::ObjectType::UNKNOWN_MOVABLE},
      {"UNKNOWN_UNMOVABLE", apollo_perception_standalone::ObjectType::UNKNOWN_UNMOVABLE},
      {"PEDESTRIAN", apollo_perception_standalone::ObjectType::PEDESTRIAN},
      {"BICYCLE", apollo_perception_standalone::ObjectType::BICYCLE},
      {"VEHICLE", apollo_perception_standalone::ObjectType::VEHICLE},
  };

  ifstream ifs(path, ifstream::in);
  if (!ifs.good()) {
    AERROR << "type_list not found, use default: VEHICLE, BICYCLE, PEDESTRIAN";
    (*types) = {apollo_perception_standalone::ObjectType::VEHICLE,
                apollo_perception_standalone::ObjectType::BICYCLE,
                apollo_perception_standalone::ObjectType::PEDESTRIAN};
  } else {
    string type;
