...
#include "camera/common/util.h"
#include "camera/detector/yolo_camera_detector/util.h"

namespace apollo_perception_standalone {

using apollo_perception_standalone::util::GetAbsolutePath;
...
bool YoloCameraDetector::Init(const CameraDetectorInitOptions &options) {
  AERROR << "YoloCameraDetector initialize from GFLAGS not available!";
  return false;
  ...
  // CHECK(util::GetProtoFromASCIIFile(yolo_config, &yolo_param_));
  // CHECK(util::GetProtoFromASCIIFile(lane_config, &lane_param_));
...
bool YoloCameraDetector::Init(std::string &file_path_config, 
  const CameraDetectorInitOptions &options) {
  // load yolo camera detector config file to proto
  std::string file_path_config_yolo_cam_detector = 
    file_path_config + "/config/camera/yolo_camera_detector_config.pb.txt";
  CHECK(GetProtoFromFile(file_path_config, &config_));

  // const string &yolo_root = config_.yolo_root();
  const string yolo_root = file_path_config +
     "/model/yolo_camera_detector/";
  const string yolo_config = GetAbsolutePath(yolo_root,
                                             FLAGS_yolo_config_filename);
  const string lane_config = GetAbsolutePath(yolo_root, "lane.pt");
...
void YoloCameraDetector::init_anchor(const string &yolo_root) {
  ...
  AINFO << " model root: " << model_root;
  AINFO << " anchors_file " << anchors_file;
...
void YoloCameraDetector::load_intrinsic(
  ...
  if (options.intrinsic == nullptr) {
    AWARN << "YoloCameraDetector options.intrinsic is nullptr. Use default image size 1080P";
    image_height_ = 1080;
    image_width_ = 1920;
    // AWARN << "YoloCameraDetector options.intrinsic is nullptr. Use default image size 720P";
    // image_height_ = 720;
    // image_width_ = 1280;
  } else {
...
bool YoloCameraDetector::init_cnn(const string &yolo_root) {
  ...
  AINFO << " feature_file: " << feature_file;
  ...
  CHECK(util::GetProtoFromASCIIFile(feature_file, &feat_param));
  ...
  // AINFO << "feat_param.has_remap_model(): " << feat_param.has_remap_model();
  // getchar();
