...
bool GeometryCameraConverter::Init() {
  AERROR << "GeometryCameraConverter initialize from GFLAGS not available!";
  return false;
...
bool GeometryCameraConverter::Init(std::string &file_path_config) {
  std::string file_path_config_geo_cam_converter = 
          file_path_config + "/config/camera/geometry_camera_converter_config.pb.txt";
  if (!GetProtoFromFile(file_path_config_geo_cam_converter, &config_)) {
    AERROR << "Cannot get config proto from file: "
           << file_path_config_geo_cam_converter;
    return false;
  }
...
bool GeometryCameraConverter::LoadCameraIntrinsics(
  ...
  if (!node["resize_factor"]) {
    AERROR << "load resize factor failed. file_path:" << file_path;
    return false;
  }
  double resize_factor = node["resize_factor"].as<double>();
