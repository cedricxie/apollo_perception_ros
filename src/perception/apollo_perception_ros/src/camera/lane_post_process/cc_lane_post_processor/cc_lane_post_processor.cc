...
#include "camera/lane_post_process/cc_lane_post_processor/cc_lane_post_processor.h"
...
bool CCLanePostProcessor::Init() {
  // 1. get model config
  CHECK(GetProtoFromFile(FLAGS_lane_post_process_config, &config_));
  ...
  const CameraCalibrationPtr camera_calibration =
      calibration_config_manager->get_camera_calibration(cam_idx_);
