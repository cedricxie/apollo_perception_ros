...
#ifndef _APOLLO_PERCEPTION_STANDALONE_LIDAR_CNN_SEGMENTATION_CLUSTER2D_H_
#define _APOLLO_PERCEPTION_STANDALONE_LIDAR_CNN_SEGMENTATION_CLUSTER2D_H_
...
#include "lidar/util/cnn_segmentation_util.h"

namespace apollo_perception_standalone {
namespace cnnseg {

using apollo_perception_standalone::util::DisjointSetFind;
using apollo_perception_standalone::util::DisjointSetMakeSet;
using apollo_perception_standalone::util::DisjointSetUnion;
 ...
 public:
  ...
  void Cluster(const caffe::Blob<float>& category_pt_blob,
    ...
    for (size_t i = 0; i < valid_indices_in_pc_->size(); ++i) {
      ...
      const pcl_util::Point& point = pc_ptr_->points[point_id];
  ...
  void Filter(const caffe::Blob<float>& confidence_pt_blob,
    ...
    for (size_t obstacle_id = 0; obstacle_id < obstacles_.size();
      ...
      obs->cloud.reset(new apollo_perception_standalone::pcl_util::PointCloud);
    }
  }
  ...
  //    }
  //    ADEBUG << std::endl;
      std::shared_ptr<Object> out_obj(new apollo_perception_standalone::Object);
 ...
 private:
  ...
  ObjectType GetObjectType(const MetaType meta_type_id) {
    switch (meta_type_id) {
      ...
      default: {
        std::cout << "Undefined ObjectType output by CNNSeg model." << std::endl;
        return ObjectType::UNKNOWN;
      }
    }
  }
  ...
  pcl_util::PointCloudPtr pc_ptr_;
