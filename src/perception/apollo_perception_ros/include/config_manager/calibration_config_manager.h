...
struct CameraCoeffient {
 ...
 private:
  ...
  bool init_camera_intrinsic_matrix_and_distort_params(
      const std::string& camera_intrinsic_file, int cam_idx);
...
class CalibrationConfigManager {
 public:
  ...
  inline CameraCalibrationPtr get_camera_calibration(int i) {
    return camera_calibration_[i];
  }
 ...
 private:
  ...
  std::vector<CameraCalibrationPtr> camera_calibration_;
